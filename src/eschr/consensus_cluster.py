...
from _base_clustering_utils import run_la_clustering #_base_clustering_utils _leiden
from _prune_features import calc_highly_variable_genes, calc_pca #ADD BACK PRECEDING DOTS
...
def consensus_cluster_leiden(in_args):
    ...
    hard_clusters, soft_membership_matrix = get_hard_soft_clusters(n, clustering, bipartite)  
...
def ensemble(zarr_loc,
    ...
    hyperparam_iterator = [[k_range, la_res_range, metric] for x in range(ensemble_size)]
...
def consensus_cluster(
    ...
    la_res_range = (int(la_res_range[0]), int(la_res_range[1])) #, per_iter_clust_assigns
    bipartite = ensemble(zarr_loc=zarr_loc,
                        reduction=reduction,
                        metric=metric,
    ...
    adata.obs["cell_conf_score"] = np.max(soft_membership_matrix, axis=1)
